...
`taxize` offers interactive prompts when using `get_*()` functions
(e.g.,
[`get_tsn()`](https://docs.ropensci.org/taxize/reference/get_tsn.md)).
These prompts make it easy in interactive use to select choices when
there are more than one match found.

However, to make your code reproducible you don’t want interactive
prompts.
...
by number only (not by a name itself). Here, we want the first 3
records:
...
Thus, for full programmatic usage, you can specify a single row, if you
happen to know which one you want:
...
In reality it is unlikely you’ll know which row you want, unless perhaps
you just want one result from each query, regardless of what it is.

### underscore methods

A better fit for programmatic use are underscore methods. Each `get_*()`
function has a sister method with and trailing underscore, e.g.,
[`get_tsn()`](https://docs.ropensci.org/taxize/reference/get_tsn.md) and
...
You can also combine `rows` parameter with underscore functions, as a
single number of a range of numbers:
...
All `get_*()` functions have associated `as.*()` functions (e.g.,
[`get_tsn()`](https://docs.ropensci.org/taxize/reference/get_tsn.md) and
[`as.tsn()`](https://docs.ropensci.org/taxize/reference/get_tsn.md)).

Many `taxize` functions use taxonomic identifier classes (S3 objects)
that are the output of `get_*()` functions. `as.*()` methods make it
easy to make the required S3 taxonomic identifier classes if you already
know the identifier. For example:
...
And you can do the same for character, or list inputs - depending on the
data source.

The above
[`as.tsn()`](https://docs.ropensci.org/taxize/reference/get_tsn.md)
examples have the parameter `check = TRUE`, meaning we ping the data
source web service to make sure the identifier exists. You can skip that
check if you like by setting `check = FALSE`, and the result is returned
much faster:
...
With the output of `as.*()` methods, you can then proceed with other
`taxize` functions.

## gnr_resolve

Some functions in `taxize` are meant specifically for name cleaning. One
of those is
[`gnr_resolve()`](https://docs.ropensci.org/taxize/reference/gnr_resolve.md).

[`gnr_resolve()`](https://docs.ropensci.org/taxize/reference/gnr_resolve.md)
doesn’t provide prompts as do `get_*()` functions, but instead return
data.frame’s. So we don’t face the same problem, and can use
[`gnr_resolve()`](https://docs.ropensci.org/taxize/reference/gnr_resolve.md)
...
Some other functions in `taxize` use `get_*()` functions internally
(e.g.,
[`classification()`](https://docs.ropensci.org/taxize/reference/classification.md)),
but you can can generally pass on parameters to the `get_*()` functions
internally.

## Feedback?

Let us know if you have ideas for better ways to do programmatic name
cleaning at <https://github.com/ropensci/taxize/issues> or
